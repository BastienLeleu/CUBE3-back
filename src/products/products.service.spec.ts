...
describe('ProductsService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should apply explicit sorting before pagination (skip/take)', async () => {
      mockQueryBuilder.getManyAndCount.mockResolvedValue([[], 0]);
      await service.findAll({});
      ...
      const orderByOrder = mockQueryBuilder.orderBy.mock.invocationCallOrder[0];
      const skipOrder = mockQueryBuilder.skip.mock.invocationCallOrder[0];
      expect(orderByOrder).toBeLessThan(skipOrder);
    });

    it('should cap the limit parameter to MAX_LIMIT (100) for security', async () => {
      mockQueryBuilder.getManyAndCount.mockResolvedValue([[], 0]);
      const excessiveLimit = 10000;
      await service.findAll({ limit: excessiveLimit });

      // The take method should be called with 100 instead of 10000
      expect(mockQueryBuilder.take).toHaveBeenCalledWith(100);
      expect(mockQueryBuilder.skip).toHaveBeenCalledWith(0); // (1-1) * 100 = 0
    });
  });
});
